...
class DashboardController {
                ...
                Post.count({})
                    .then((result_2) => {
                        ...
                        if (result_2 != result[0].posts_all.length) {
                                ...
                                .then((allID_raw) => {
                                    ...
                                    for (ele in result) {
                                        ...
                                        for (idEle in allID) {
                                            if (
                                                result_2 >
                                                result[ele].posts_all.length
                                            ) {
                                                if (
                                                    result[ele][idEle] ==
                                                    undefined
                                                ) {
                                                    result[ele].posts_checked[
                                                        idEle
                                                    ] = '0';
                                                }
                                            }
                                            if (
                                                ...
                                                let tempObj =
                                                    result[ele].posts_checked;
                                                for (let member in result[ele]
                                                    .posts_checked)
                                                    delete result[ele]
                                                        .posts_checked[member];
                                                if (tempObj[idEle] == '1') {
                                                    result[ele].posts_checked[
                                                        idEle
                                                    ] = '1';
                                        ...
                                        switch (result[ele].name) {
                                            case 'top':
                                                top_list = mongooseToObject(
                                                    result[ele],
                                                );
                                                break;

                                            case 'hot':
                                                hot_list = mongooseToObject(
                                                    result[ele],
                                                );
                                            // break;
                                        }
                                        Post_special_list.findOneAndUpdate(
                                            { name: result[ele].name },
                                            result[ele],
                                        )
                                            .then((saveListResult) => {
                                    ...
                                    res.redirect('dashboard/post-manage');
                            ...
                            res.render('dashboard/post-manage', {
                                topList: top_list,
                                hotList: hot_list,
                                deletedPost,
                            });
    ...
    submit_video(req, res, next) {
        Video.updateOne({ code: 1 }, req.body, { upsert: 1 }).then(() => {
            res.send('1');
        });
    }
}
